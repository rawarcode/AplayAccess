...
export default function Rooms() {
  ...
  const renderRoomCard = (r) => (
    // Card is presentational. The Details and Book Now buttons inside
    // are the only interactive elements — that drops the previous
    // role="button" wrapper that had nested buttons (invalid ARIA + 3
    // tab stops per card). Hover/lift styles stay so the card still
    // *feels* interactive; the Details button is the explicit target.
    <div key={r.id ?? r.name}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex gap-2">
          <button onClick={() => r.id && openDetails(r.id)}
            type="button"
            disabled={!r.id}
            aria-label={r.id ? `View details for ${r.name}` : undefined}
            className="flex-1 border border-sky-200 text-sky-700 hover:border-sky-400 hover:bg-sky-50 px-3 py-2.5 rounded-xl text-sm font-semibold min-h-11 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-1 disabled:opacity-50">
            Details
          </button>
          <button onClick={() => requestBooking(r.name)}
            type="button"
            className="flex-1 bg-sky-600 hover:bg-sky-700 text-white px-3 py-2.5 rounded-xl text-sm font-semibold shadow hover:shadow-md min-h-11 transition-[background-color,box-shadow] focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-1">
            Book Now
          </button>
        </div>
      </div>
              ...
              {roomCards.length === 0 ? (
                <div className="text-center py-20">
                  <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-4">
                    <i className="fas fa-bed text-3xl text-slate-400" aria-hidden="true" />
                  </div>
                  <h3 className="text-lg font-semibold text-slate-700 mb-2">No {activeTab === "all" ? "rooms" : CATEGORY_TABS.find(t => t.key === activeTab)?.label.toLowerCase() || "rooms"} found</h3>
                  <p className="text-slate-600 text-sm mb-4">Try selecting a different category above.</p>
                  <button
