...
function printReport({ bookings, active, revenue, avgVal, nights, period, roomTypes, roomRevenue, roomBookings }) {
  ...
  const bookingRows = bookings.map(b => `<tr${b.status==='Cancelled'?' style="color:#94a3b8"':''}><td>${b.guest}</td><td>${b.room}</td><td>${b.payment||'—'}</td><td style="text-align:center">${b.nights}</td><td style="text-align:right">${b.status==='Cancelled'?`<s>₱${fmtN(b.total)}</s>`:`₱${fmtN(b.total)}`}</td><td style="text-align:right;color:#92400e">${Number(b.entranceFee)>0?`₱${fmtN(b.entranceFee)}`:'—'}</td><td>${statusBadge(b.status)}</td></tr>`).join('');
