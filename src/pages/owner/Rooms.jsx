...
export default function AdminRooms() {
  ...
  return (
    ...
    <div className="p-6 space-y-6">
          ...
          return (
            <>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                ...
                <button type="button" onClick={() => setViewRoom(null)} className="h-11 w-11 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-600 hover:text-slate-800 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2" aria-label="Close">
      ...
      {/* ── Edit / Create Modal ── */}
      <Modal open={modalOpen} onClose={guardedCloseModal} maxWidth="max-w-2xl" label={editing ? `Edit room — ${editing.name}` : 'Add new room'}>
        <form onSubmit={requestSaveRoom} ref={formRef} className="max-h-[85vh] overflow-y-auto">
                    ...
                    <div className="divide-y divide-slate-200 rounded-lg border border-slate-200 bg-white">
                      {addonCatalog.map(cat => {
                        ...
                        const rowBg = relation === 'package'  ? 'bg-emerald-50/60'
                                    ...
                                    : '';
                        return (
                          // Row wraps on narrow widths so the segmented
                          // picker + price/qty drop to a second line
                          // instead of overflowing. Heights bumped to
                          // min-h-10 / min-h-9 across controls for touch
                          // use without making the modal absurdly tall;
                          // focus rings on every interactive child.
                          <div key={cat.id}
                            className={`flex flex-wrap items-center gap-2 sm:gap-3 px-3 py-2 ${rowBg}`}>
                            <i className={`fas ${cat.icon || 'fa-tag'} text-slate-500 w-5 text-center text-sm`} aria-hidden="true"></i>
                            <span className="font-medium text-slate-700 text-sm flex-1 min-w-[7rem] truncate">{cat.name}</span>

                            {/* Status picker — bumped to min-h-9 segments
                                with text-xs + focus rings */}
                            <div className="inline-flex rounded-md overflow-hidden border border-slate-300 text-xs font-semibold shrink-0">
                              {[
                                { key: null,        label: 'None' },
                                      ...
                                      active
                                        ? opt.key === 'package'  ? 'bg-emerald-600 text-white focus-visible:ring-emerald-500' :
                                          opt.key === 'optional' ? 'bg-sky-600 text-white focus-visible:ring-sky-500' :
                                                                   'bg-slate-600 text-white focus-visible:ring-slate-500'
                                        : 'bg-white text-slate-700 hover:bg-slate-50 focus-visible:ring-slate-400'
                                    }`}>
                                    {opt.label}
                                  </button>
                                );
                            ...
                            {/* Price + qty — bumped heights, slate-300
                                borders for AA contrast, focus rings */}
          ...
          <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex justify-end gap-3">
            ...
            <button type="submit" disabled={saving}
              className="inline-flex items-center justify-center min-h-11 px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm font-semibold disabled:opacity-60 shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2">
