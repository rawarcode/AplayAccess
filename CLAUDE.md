...
- **Add-on inventory:** `addons.max_qty` is the total physical units the resort owns (not a per-booking cap). The overlap check in `Admin\AmenityController` sums allocated qty across bookings whose windows overlap — `BookingAmenity::allocatedInWindow()` is the shared helper. Live remaining counts for staff-facing pickers (walk-in form, booking detail) come from `GET /api/admin/addons/availability` — two modes: `?booking_id=X` (existing booking's window, excluding its own allocation) or `?date=Y&booking_type=Z&hour=W` (computed from scratch for a walk-in slot).
