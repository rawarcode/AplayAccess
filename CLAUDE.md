...
Admin was reintroduced as a distinct role in 2026-04-24 (commits `3030b6b` → `73f7757` → `1365f60`). It sits between front desk and owner: admin runs day-to-day operations (content, reviews, messages, promo codes, newsletter, addons catalog, auto-replies) but cannot change rates, manage staff, view revenue, or access the audit log. 2026-04-25 expanded that scope with a "limited oversight" tier: admin can read the activity log, toggle a `front_desk` user's active flag (no full user CRUD), change a room's availability_status (no rate / capacity / name edits), and toggle `is_active` on an add-on (no create / rename / reprice / delete). Strategic CRUD (rooms create/delete/rename/reprice, addons CRUD, staff CRUD, owner-account changes) stays owner-only. Full permission matrix lives in `docs/roles.xlsx`. Owner keeps every admin capability, so an owner logged in can also navigate `/admin/*` directly.
...
- `admin_or_owner` — admin + owner. Day-to-day management (content, reviews, addons catalog, auto-replies, promo codes, newsletter, guests list, announcements). Also covers GET /admin/users + GET /admin/rooms + GET /admin/addons + GET /admin/history (read-only listings) and three narrow toggle endpoints: `PATCH /admin/users/{id}/toggle-active` (front_desk only — backend rejects other roles + self), `PATCH /admin/rooms/{id}/availability` (status-only), `PATCH /admin/addons/{id}/toggle-active`. Owner can call these too.
...
- `owner_role` — owner only. Staff management (full CRUD), analytics, financial reports.
...
- **Chromium propagates the opener's CSP to `about:blank` popups.** Any `window.open('', '_blank')` + `document.write(html)` pattern that embeds an inline `<script>window.print()</script>` in the written HTML will silently fail under our current CSP (script-src has no `'unsafe-inline'` since the P1 audit pass). The popup opens and content renders, but the print dialog never fires. Fix: drop the inline script and invoke `w.print()` from the opener via `setTimeout(..., 250)` after `w.document.close()` so the popup has a tick to lay out first. All three print callers (`frontdesk/Reports.jsx`, `owner/Reports.jsx`, `owner/Transactions.jsx`) follow this pattern now — if you add a fourth print surface, copy it, don't reintroduce the inline script.
...
- **`/admin/users` and `/admin/activity-log` reuse owner pages with role-aware rendering.** Both paths in `App.jsx` mount `OwnerUsers` / `OwnerActivityLog` under the admin shell — there is no separate admin component. `OwnerUsers` reads `useAuth().user.role === 'admin'` into an `isAdminView` flag and gates owner-only UI behind `{!isAdminView && (...)}`: Add User button + Ctrl+N shortcut, bulk-action bar, select-all + per-row checkboxes, Edit pen, Edit button in the view modal. `canDelete` returns false for admin (no delete trash icon). `canToggleActive` additionally requires `u.role === 'front_desk'` for admin (matches backend `toggleStaffActive` 403 contract). The role filter chips collapse to a single "Front Desk" pill for admin and `filterRole` initializes to `'front_desk'` so the list is pre-scoped. `handleToggleActive` routes through `toggleStaffActive` (PATCH narrow) for admin and `updateAdminUser` (full update) for owner. If a new owner-only feature lands on this page, gate it behind `isAdminView` the same way; if a new toggle endpoint is added for admin's tier, mirror the routing in `handleToggleActive`. Activity Log doesn't need gating — it's read-only by design.
