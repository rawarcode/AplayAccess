...
- **Walk-in account linking is email-or-phone, role-scoped.** `Admin\WalkInController::store` resolves `user_id` in this order: (1) email match against `users.email`; (2) phone match against `users.phone` digits-only, scoped to `role='guest'` so an accidental phone collision with staff/owner can't attach a walk-in to a non-guest; (3) create a brand-new `users` row (random password, role=guest) when an email is provided but no match exists; (4) anonymous walk-in with `user_id = null` when neither email nor phone match. The phone fallback is what makes `WalkIn.jsx` "Returning guest" search reliable — staff can pick a past walk-in by phone alone and the new booking still surfaces in that guest's `My Bookings` on next login. The phone-digit comparison uses `REGEXP_REPLACE(phone, '[^0-9]', '')` server-side so `+63 908 191 4721` and `09081914721` resolve to the same row. (Frontend lookup pulls from `/api/admin/guests` + `/api/admin/bookings` parsed walk-ins, deduped by digits-only phone, sorted by lastVisit desc.)
...
- **Messaging:** One-thread-per-guest — `POST /api/messages` from a guest who already has a thread appends to it rather than creating a new root. Staff can initiate threads via `POST /api/admin/messages/compose`. Abusive guests can be flagged via `POST /api/admin/users/{user}/toggle-messaging-block`. Rate limits: 5 new / 10 replies per minute. Plus content-level anti-spam in `MessageController::assertNotSpam()` (called from both `store()` and `reply()`): URL flood (>3 links per message) → 422; duplicate body sent by the same user within the last 10 minutes (case + whitespace normalized) → 429. Both reject pre-persistence so spam never reaches the staff inbox or the auto-reply engine.
